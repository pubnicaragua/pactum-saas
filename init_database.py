...
async def seed_initial_data():
    ...
    # Create Miguel Estanga (Frontend Developer - TEAM_MEMBER for Alma IA)
    miguel_user_id = str(uuid.uuid4())
    miguel_user_doc = {
        "id": miguel_user_id,
        "email": "miguel.estanga@almaia.com",
        "password": hash_password("MiguelAlma2026!"),
        "name": "Miguel Alejandro Estanga",
        "role": "TEAM_MEMBER",
        "company_id": pactum_company_id,
        "status": "active",
    ...
    print("✅ Usuario TEAM_MEMBER creado: miguel.estanga@almaia.com (Frontend)")
    
    # Create Jonathan Roque (Backend Developer - TEAM_MEMBER for Alma IA)
    jroque_user_id = str(uuid.uuid4())
    jroque_user_doc = {
        "id": jroque_user_id,
        "email": "jonathan.roque@almaia.com",
        "password": hash_password("JRoqueAlma2026!"),
        "name": "Jonathan Roque",
        "role": "TEAM_MEMBER",
        "company_id": pactum_company_id,
        "status": "active",
    ...
    print("✅ Usuario TEAM_MEMBER creado: jonathan.roque@almaia.com (Backend)")
    
    # Update Alma IA project to include Miguel and Jonathan
    await db.projects.update_one(
        {"id": alma_project_id},
        {"$set": {"assigned_users": [alma_user_id, miguel_user_id, jroque_user_id]}}
    )
    print("✅ Miguel y Jonathan asignados al proyecto Alma IA")
    
    # Create Frontend tasks for Miguel
    print("📱 Creando tareas de Frontend para Miguel...")
    frontend_tasks = [
        {"title": "HomeScreen2 (Gamificado) - Diseño y Maquetación", "description": "Implementar pantalla principal gamificada con elementos visuales atractivos, badges, progreso del usuario y animaciones. Incluir screenshots del progreso cada hora.", "priority": "high", "estimated_hours": 1, "status": "backlog", "assigned_to": miguel_user_id},
        {"title": "Módulo Diario - Frontend", "description": "Crear interfaz para registro diario del usuario. Incluir formularios, validaciones y feedback visual. Documentar endpoints utilizados y subir screenshots.", "priority": "high", "estimated_hours": 1, "status": "backlog", "assigned_to": miguel_user_id},
        {"title": "Módulo SOS - Frontend", "description": "Implementar botón de emergencia SOS con grabación de audio/video, envío de texto y geolocalización. Probar en iOS y Android. Screenshots cada hora.", "priority": "urgent", "estimated_hours": 1, "status": "backlog", "assigned_to": miguel_user_id},
        {"title": "Módulo PyR (Preguntas y Respuestas) - Frontend", "description": "Crear interfaz para sistema de preguntas diarias (Emoción, Neurodivergencia, Patologías). Validar que todas las preguntas se muestren correctamente. Documentar endpoints.", "priority": "high", "estimated_hours": 1, "status": "backlog", "assigned_to": miguel_user_id},
        {"title": "Módulo AVISOS - Frontend", "description": "Implementar sistema de notificaciones y avisos. Diferenciar visualmente avisos simples de encuestas (colores + iconos). Ordenar descendente (más reciente primero). Screenshots.", "priority": "high", "estimated_hours": 1, "status": "backlog", "assigned_to": miguel_user_id},
        {"title": "Módulo ENCUESTAS - Frontend", "description": "Crear interfaz para encuestas dinámicas. Permitir 2-4 opciones (no obligar 4ta opción). Navegación desde avisos a encuestas. Manejo de errores al guardar. Screenshots.", "priority": "high", "estimated_hours": 1, "status": "backlog", "assigned_to": miguel_user_id},
        {"title": "Módulo BENEFICIOS - Frontend", "description": "Implementar pantalla de beneficios para usuarios. Diseño atractivo con cards, imágenes y descripciones. Documentar endpoints y subir screenshots cada hora.", "priority": "medium", "estimated_hours": 1, "status": "backlog", "assigned_to": miguel_user_id},
        {"title": "Registro Semanal - Frontend", "description": "Crear interfaz para registro semanal de actividades. Calendario interactivo, gráficas de progreso. Validar que no se rompa la gráfica al seleccionar fechas. Screenshots.", "priority": "medium", "estimated_hours": 1, "status": "backlog", "assigned_to": miguel_user_id},
        {"title": "Pruebas y Validaciones Frontend - Miguel", "description": "Realizar pruebas exhaustivas de todos los módulos frontend implementados. Detectar errores, validar flujos de usuario, verificar responsive design en iOS y Android. Documentar bugs encontrados antes de entregar.", "priority": "high", "estimated_hours": 2, "status": "backlog", "assigned_to": miguel_user_id}
    ]
    ...
    for task_data in frontend_tasks:
        ...
        await db.tasks.insert_one(task_doc)
    print(f"✅ {len(frontend_tasks)} tareas de Frontend creadas para Miguel")
    
    # Create Backend tasks for Jonathan
    print("⚙️  Creando tareas de Backend para Jonathan...")
    backend_tasks = [
        {"title": "Validar funcionalidad en App y Web", "description": "Verificar que todos los endpoints funcionen correctamente en ambas plataformas (App móvil y Web). Documentar diferencias y problemas encontrados. Screenshots de pruebas.", "priority": "urgent", "estimated_hours": 1, "status": "backlog", "assigned_to": jroque_user_id},
        {"title": "Garantizar Preguntas Diarias - Sistema automático", "description": "Implementar sistema que garantice que las 3 preguntas diarias (Emoción, Neurodivergencia, Patologías) se muestren correctamente. Validar nombres exactos en BD vs App.", "priority": "urgent", "estimated_hours": 1, "status": "backlog", "assigned_to": jroque_user_id},
        {"title": "Informes automáticos - Inicio y fin de día", "description": "Crear sistema de informes automáticos que se envíen al inicio y final del día con status del sistema, usuarios activos, errores y métricas clave.", "priority": "high", "estimated_hours": 1, "status": "backlog", "assigned_to": jroque_user_id},
        {"title": "Garantizar operación Dev y Prod con datos correctos", "description": "Validar que ambos ambientes (Desarrollo y Producción) operen correctamente con sus respectivos datos. Documentar configuraciones y diferencias.", "priority": "high", "estimated_hours": 1, "status": "backlog", "assigned_to": jroque_user_id},
        {"title": "Fix: Endpoint preguntas - Validar nombres exactos", "description": "Revisar endpoint de preguntas diarias. El problema reportado: solo aparecen 2 de 3 preguntas (falta Patologías). Validar que nombres en BD coincidan exactamente con lo que espera la App.", "priority": "urgent", "estimated_hours": 1, "status": "backlog", "assigned_to": jroque_user_id},
        {"title": "Fix: SOS Audio - Visualización en Web", "description": "Corregir bug donde audios enviados por SOS desde App no se visualizan en Web. Validar almacenamiento y recuperación de archivos de audio.", "priority": "high", "estimated_hours": 1, "status": "backlog", "assigned_to": jroque_user_id},
        {"title": "Fix: Encuestas - Error al guardar segunda respuesta", "description": "Investigar y corregir error que ocurre al guardar la segunda respuesta de una encuesta. Primera respuesta OK, segunda falla. Logs y screenshots.", "priority": "high", "estimated_hours": 1, "status": "backlog", "assigned_to": jroque_user_id},
        {"title": "Optimización: Cambio de contraseña", "description": "Revisar flujo de cambio de contraseña. Reportado: indica que guardó OK pero luego no permite login. Validar hash y actualización en BD.", "priority": "medium", "estimated_hours": 1, "status": "backlog", "assigned_to": jroque_user_id},
        {"title": "Pruebas y Validaciones Backend - Jonathan", "description": "Realizar pruebas exhaustivas de todos los endpoints y funcionalidades backend. Validar respuestas, tiempos de respuesta, manejo de errores. Probar en Dev y Prod. Documentar bugs encontrados antes de entregar.", "priority": "high", "estimated_hours": 2, "status": "backlog", "assigned_to": jroque_user_id}
    ]
    ...
    for task_data in backend_tasks:
        ...
        await db.tasks.insert_one(task_doc)
    print(f"✅ {len(backend_tasks)} tareas de Backend creadas para Jonathan")
    
    # Create Issues/Bugs tasks
    print("🐛 Creando tareas de Issues reportados...")
    issues_tasks = [
        {"title": "BUG iOS: Gráfica de actividades se rompe al seleccionar fecha", "description": "Reproducir y corregir bug donde la gráfica en sección de actividades se rompe al escoger una fecha. Solo reportado en iOS. Screenshots del error.", "priority": "high", "estimated_hours": 1, "status": "backlog", "assigned_to": miguel_user_id},
        {"title": "BUG iOS: SOS exige texto cuando solo se grabó voz", "description": "Corregir validación en SOS. Si usuario solo graba voz, no debería exigir texto obligatorio. Validar en iOS y Android.", "priority": "high", "estimated_hours": 1, "status": "backlog", "assigned_to": miguel_user_id},
        {"title": "BUG iOS: Cerrar sesión cierra la app en lugar de ir a login", "description": "Al cerrar sesión, la app se cierra completamente. Debería redirigir a pantalla de login. Corregir navegación.", "priority": "medium", "estimated_hours": 1, "status": "backlog", "assigned_to": miguel_user_id},
        {"title": "UX: Avisos sin título muestran 'Sin título'", "description": "Mejorar UX: cuando un aviso no tiene título, mostrar primeras palabras del contenido en lugar de 'Sin título'.", "priority": "low", "estimated_hours": 1, "status": "backlog", "assigned_to": miguel_user_id},
        {"title": "UX: Ordenar avisos descendente (más reciente primero)", "description": "Cambiar orden de avisos para que el más reciente aparezca primero (orden descendente por fecha).", "priority": "medium", "estimated_hours": 1, "status": "backlog", "assigned_to": jroque_user_id},
        {"title": "UX: Contador de mensajes no leídos no cuadra con total", "description": "Investigar discrepancia entre total de mensajes y contador de no leídos. Validar lógica de marcado como leído.", "priority": "medium", "estimated_hours": 1, "status": "backlog", "assigned_to": jroque_user_id},
        {"title": "UX: Diferenciar avisos de encuestas visualmente", "description": "Agregar colores e iconos diferentes para distinguir avisos simples de encuestas. Mejorar navegación desde aviso a encuesta (doble clic).", "priority": "medium", "estimated_hours": 1, "status": "backlog", "assigned_to": miguel_user_id},
        {"title": "UX: Encuestas no deben obligar 4ta opción", "description": "Modificar formulario de creación de encuestas para permitir 2-4 opciones sin obligar la cuarta opción.", "priority": "medium", "estimated_hours": 1, "status": "backlog", "assigned_to": miguel_user_id},
        {"title": "BUG Android: Encuestas no muestran preguntas", "description": "Reportado en Android: las encuestas llegan pero no se muestran las preguntas. Validar parsing y renderizado de preguntas.", "priority": "high", "estimated_hours": 1, "status": "backlog", "assigned_to": miguel_user_id},
        {"title": "UX: Texto largo 'denunciémoslo' se corta", "description": "Ajustar UI para que palabra 'denunciémoslo' aparezca completa. Considerar cambiar a 'Denuncia' o ajustar ancho del contenedor.", "priority": "low", "estimated_hours": 1, "status": "backlog", "assigned_to": miguel_user_id},
        {"title": "UX: Menú inferior desaparece en algunas pantallas", "description": "El menú de navegación inferior (Home, Agenda, etc) desaparece en algunas pantallas. Hacer que sea persistente en todas las vistas principales.", "priority": "medium", "estimated_hours": 1, "status": "backlog", "assigned_to": miguel_user_id},
        {"title": "UX: Inconsistencia en tonos de azul", "description": "Estandarizar paleta de colores. Los azules no son iguales en diferentes secciones. Crear guía de estilos y aplicar consistentemente.", "priority": "low", "estimated_hours": 1, "status": "backlog", "assigned_to": miguel_user_id}
    ]
    ...
    for task_data in issues_tasks:
        ...
        await db.tasks.insert_one(task_doc)
    print(f"✅ {len(issues_tasks)} tareas de Issues creadas")
    
    total_alma_tasks = len(frontend_tasks) + len(backend_tasks) + len(issues_tasks)
    print(f"✅ TOTAL ALMA IA: {total_alma_tasks} tareas creadas (9 Frontend + 9 Backend + 12 Issues)")
    ...
    print("�‍💻 MIGUEL ESTANGA (TEAM_MEMBER - Frontend Alma IA):")
    print("   Email:    miguel.estanga@almaia.com")
    print("   Password: MiguelAlma2026!")
    print("   Tareas:   9 Frontend + Issues UX\n")
    
    print("👨‍💻 JONATHAN ROQUE (TEAM_MEMBER - Backend Alma IA):")
    print("   Email:    jonathan.roque@almaia.com")
    print("   Password: JRoqueAlma2026!")
    print("   Tareas:   9 Backend + Validaciones\n")
